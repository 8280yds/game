...
public class VectorLine {
	public static string Version () {
		return "Vectrosity version 4.0.2";
	}
	...
	public bool active {
		...
		set {
			m_active = value;
			if (m_vectorObject != null) {
				m_vectorObject.SetActive (m_active);
			}
		}
	}
	float m_capLength;
	...
	public Joins joins {
		...
		set {
			...
			if (m_joins != Joins.Fill && m_fillObjectSet) {
				m_fillRenderer.gameObject.SetActive (false);
			}
		}
	}
	bool m_isPoints;
	...
	private void SetEndCapColors () {		
		...
		m_capRenderer.SetVertices (m_capVertices, 8);
	}
	...
	public void SetColor (Color color, int startIndex, int endIndex) {
		...
		m_canvasRenderer.SetVertices (m_UIVertices, m_vertexCount - adjustEnd*4);
		if (m_joins == Joins.Fill) {
			SetFillColors();
		}
	}
	...
	private void SetFillColors () {
		...
		m_fillRenderer.SetVertices (m_fillVertices, m_vertexCount - adjustEnd*4 - (connectFirstAndLast? 0 : 4));
	}
	...
	private void DrawEndCap (bool draw3D) {
		if (m_capType <= EndCap.Mirror) {
			...
			if (!draw3D) {
				...
				m_UIVertices[vIndex+1].position = cam3D.ScreenToWorldPoint (m_screenPoints[vIndex+1] + d2);
			}
		...
		if (m_capType >= EndCap.Both) {
			...
			if (!draw3D) {
				...
				m_UIVertices[vIndex-3].position = cam3D.ScreenToWorldPoint (m_screenPoints[vIndex-1] + d2);
	...
	private void SetupDrawStartEnd (out int start, out int end, bool clearVertices) {
		...
		if (m_drawEnd < m_pointsCount - 1) {
			...
			if (!m_continuous && !m_isPoints) {
				adjustEnd += ((m_pointsCount - 1) - end) / 2;
			}
			else {
	...
	public void Draw3D () {
		...
		int start, end, add, widthIdx = 0;
		SetupDrawStartEnd (out start, out end, true);
		Matrix4x4 thisMatrix;
		bool useMatrix = UseMatrix (out thisMatrix);
		
		int idx, widthIdxAdd = 0;
		if (m_lineWidths.Length > 1) {
			widthIdx = start;
			widthIdxAdd = 1;
		}
		...
		Vector3 thisLine, px = Vector3.zero;
		var pos1 = Vector3.zero;
		var pos2 = Vector3.zero;
		for (int i = start; i < end; i += add) {
			if (m_points3[i].x == m_points3[i+1].x && m_points3[i].y == m_points3[i+1].y && m_points3[i].z == m_points3[i+1].z) {
				SkipQuad (ref idx, ref widthIdx, ref widthIdxAdd);
				continue;
			}
			if (useMatrix) {
	...
	private void WeldJoins3D (int start, int end, bool connectFirstAndLast) {
		...
		for (int i = start; i < end; i+= 4) {
			SetIntersectionPoint3D (i-4, i-3, i,   i+3);
			SetIntersectionPoint3D (i-1, i-2, i+1, i+2);
		}
	}
